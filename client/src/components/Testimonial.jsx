...
const Testimonial = () => {
    ...
    */
    ...
    return (
        <div className='px-4 sm:px-20 xl:px-32 py-12 sm:py-20 md:py-24'>
            {/* Section headline and description */}
            <div className='text-center mb-8 sm:mb-10'>
                <h2 className='text-slate-700 text-2xl sm:text-3xl md:text-[42px] font-semibold mb-2 sm:mb-3'>Loved by Creators</h2>
                <p className='text-gray-500 max-w-lg mx-auto text-xs sm:text-sm md:text-base leading-relaxed px-2'>Don't just take our word for it. Here's what our users are saying.</p>
            </div>
            ...
            */}
            <div className='flex flex-wrap justify-center gap-4 sm:gap-0'>
                {dummyTestimonialData.map((testimonial, index) => (
                    <div key={index} className='p-5 sm:p-6 md:p-8 m-0 sm:m-4 w-full sm:max-w-xs rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:-translate-y-1 transition duration-300 cursor-pointer'>
                        ...
                        <div className="flex items-center gap-0.5 sm:gap-1">
                            {Array(5).fill(0).map((_, img) => (
                                ...
                                className='w-3.5 h-3.5 sm:w-4 sm:h-4' alt="star"/>
                            ))}
                        </div>
                        {/* Testimonial content (user's review) */}
                        <p className='text-gray-500 text-xs sm:text-sm my-4 sm:my-5 leading-relaxed'>"{testimonial.content}"</p>
                        <hr className='mb-4 sm:mb-5 border-gray-300' />
                        {/* User info: profile image, name, and title */}
                        <div className='flex items-center gap-3 sm:gap-4'>
                            <img src={testimonial.image} className='w-10 h-10 sm:w-12 sm:h-12 object-cover rounded-full' alt='' />
                            <div className='text-xs sm:text-sm text-gray-600'>
                                <h3 className='font-medium'>{testimonial.name}</h3>
                                <p className='text-[10px] sm:text-xs text-gray-500'>{testimonial.title}</p>
                            </div>
                        </div>
                    </div>
                ))}
